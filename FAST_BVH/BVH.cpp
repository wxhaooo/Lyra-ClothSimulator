...
struct BVHTraversal {
  uint32_t i; // Node
  float mint; // Minimum hit time for this node.就是记录当前最小的相交系数t
  BVHTraversal() { }
...
bool BVH::getIntersection(const Ray& ray, IntersectionInfo* intersection, bool occlusion) const {
  ...
  while(stackptr>=0) {
    ...
    float nearTmp = todo[stackptr].mint;
    stackptr--;
    const BVHFlatNode &node(flatTree[ ni ]);

    // If this node is further than the closest found intersection, continue
    if(nearTmp > intersection->t)
      continue;
